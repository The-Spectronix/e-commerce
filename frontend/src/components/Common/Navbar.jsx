...
const toggleCartDrawer = () => {
  ...
  return (
    <>
     <nav className='conainer mx-auto flex items-center justify-between py-4 px-6'>
        {/* left logo */}
        <div>
            <Link to="/" className="text-2xl font-medium">Shopistry</Link>
        </div>
        {/* center navigations links */}
        <div className='hidden md:flex space-x-6'>
    <Link to="/collections/all?gender=Men" className=
    "text-gray-700 hover:text-black text-sm font-medium uppercase">Men</Link>
     <Link to="/collections/all?gender=Women" className=
    "text-gray-700 hover:text-black text-sm font-medium uppercase">Women</Link>
     <Link to="/collections/all?category=Top Wear" className=
    "text-gray-700 hover:text-black text-sm font-medium uppercase">Top Wear</Link>
     <Link to="/collections/all?category=Bottom Wear" className=
    "text-gray-700 hover:text-black text-sm font-medium uppercase">Bottom Wear</Link>
        </div>
        {/* right - Icons */}
        <div className='flex items-center space-x-4'>
            {user && user.role === "admin" && (
                <Link to="/admin" className='block bg-black text-white px-2 rounded text-sm'>
            Admin
            </Link>
            )}
        ...
        {cartItemCount > 0 && (
             <span className='absolute -top-1 bg-red text-white text-xs rounded-full px-2 py-0.5 '>{cartItemCount}</span>
        )}
